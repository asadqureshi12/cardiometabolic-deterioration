...
CVD status is assigned at cohort entry and is fixed for the duration of scoring. Three states are possible: `RECENT` (STEMI or NSTEMI within 365 days — SNOMED 401303003 / 401314000 — Band floor 2 applied, LDL target 2.0 mmol/L (77.3 mg/dL) per NICE NG238 post-ACS), `ESTABLISHED` (any CVD history: IHD, HF, AF, valve disease, MI/CABG — LDL target 2.0 mmol/L (77.3 mg/dL)), `NONE` (no CVD history — LDL target 3.0 mmol/L (116.0 mg/dL)). LDL targets are stated in mg/dL for display purposes only — all scoring computations are performed in mmol/L via deterministic conversion (mmol/L × 38.67). CVD status determines which LDL exceedance threshold is applied throughout all downstream scoring.
