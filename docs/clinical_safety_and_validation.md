...
| L-08 | Deprivation scoring formula defined but not implemented in scoring output | Band assignment | Deprivation not factored into current scores | Locked as future scope — formula (`11 - deprivation_decile`) and rationale documented in project_reference_table.md |
...
| L-11 | Synthetic variance threshold limitation | Variance instability threshold / temporal signal classification | The variance instability threshold (`0.001`) was derived from Synthea-generated monthly I-score variance. Sensitivity analysis showed stable WORSENING+UNSTABLE output across the `0.001–0.002` range within this synthetic cohort, but this does not establish real-world calibration. Synthetic EHR data may not accurately represent physiological drift, measurement variation, review frequency, or biological noise. | Threshold acceptable for synthetic proof of concept only. Real deployment would require recalibration using local EHR data, marker-specific variance distributions, laboratory/device variation data, and prospective clinical review. |
| L-12 | PARTIALLY_SUFFICIENT trajectory limitation | Temporal trajectory classification | PARTIALLY_SUFFICIENT markers use direction-only change: `SIGN(last_month_i - first_month_i)`, while DATA_SUFFICIENT markers apply marker-specific measurement or biological variation thresholds. This creates a lower-confidence asymmetry where small sparse-record increases may be labelled WORSENING despite falling within expected measurement variation. | Design intent was to avoid silently suppressing possible deterioration in sparse records. PARTIALLY_SUFFICIENT trajectory labels should not be interpreted with the same confidence as DATA_SUFFICIENT trends. In real deployment, refine by applying marker-specific error thresholds to partial records or relabelling partial-data increases as DIRECTIONAL_INCREASE rather than WORSENING. |
